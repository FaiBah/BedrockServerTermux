...
while true; do
    ...
    [ -f "$SERVER_DIR/$VERSION_FILE" ] &&
        SERVER_VERSION="$(head -n1 "$SERVER_DIR/$VERSION_FILE" 2>/dev/null || true)"
    ...
    while true; do
        ...
        case "$type" in
            ...
            1)
                ...
                COMMAND=(tar -czf "$BACKUP_ROOT/$SERVER_NAME/$FILE" -C "$SERVER_DIR" .)
                ;;

            2)
                LABEL="World data only"
                [ -d "$SERVER_DIR/worlds" ] ||
                    { err "No worlds directory found in $SERVER_NAME."; sleep 1; continue; }

                FILE="${SERVER_NAME}_worlds_$(date +%Y%m%d_%H%M%S).tar.gz"
                COMMAND=(tar -czf "$BACKUP_ROOT/$SERVER_NAME/$FILE" -C "$SERVER_DIR" worlds)
                ;;
            ...
            3)
                ...
                COMMAND=(tar -czf "$BACKUP_ROOT/$SERVER_NAME/$FILE" -C "$SERVER_DIR" "${FILES[@]}")
                ;;

            *) err "Invalid backup choice."; sleep 1; continue ;;
        esac
